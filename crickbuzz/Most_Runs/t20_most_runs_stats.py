...
driver.find_element(By.XPATH,"//a[normalize-space()='Most Runs']").click()
time.sleep(5)
most_run_by_players=driver.find_elements(By.CLASS_NAME,"cb-srs-stats-tr")
for run in most_run_by_players:
    runs=run.text
    T20I_Most_Runs.append(runs)
print(T20I_Most_Runs)



with open('T20I_most_runs.csv', 'w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(["PLAYER", "MATCHES", "INNS", "RUNS", "AVG", "SR", "4s", "6s"])
    for row in T20I_Most_Runs:
